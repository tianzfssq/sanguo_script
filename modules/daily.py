"""每日任务自动化：依次执行 打竞技场 → 打工会boss → 钓鱼。

流程：
1. 自动打竞技场（结束后自动退回主界面）
2. 自动打工会boss（结束后自动退回主界面）
3. 钓鱼（领地）（不在领地会自动进入）
4. 全部完成后返回主界面
"""
...
@register_action("daily_task", "每日任务", "主界面", "依次：打竞技场 → 打工会boss → 钓鱼")
class DailyTaskModule(BaseModule):
    def run(self, ctx: Context) -> None:
        ctx.logger.info("每日任务启动：打竞技场 → 打工会boss → 钓鱼")
